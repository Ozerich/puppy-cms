$(document).ready(function () {
    $('#filter #city').change(
        function () {
            ...
            if (city == 'Киев')
                $('#filter #price').removeAttr('disabled').html(($('#filter #kind option:selected').attr('animal') == 2 ?
                ...
                $('#filter #price').removeAttr('disabled').html('<option value="0_40000">до 40 000 руб</option>' +
                    '<option value="40001_70000">до 70 000 руб</option><option value="70001_inf">от 70 000 руб</option>');


        }).change();

    $('#filter #kind').change(function(){
        var kind = $(this).find('option:selected');

        if($(kind).attr('has_weight') == '1')
            $('#filter #weight').removeAttr('disabled').html('<option value="0_1.6">до 1.6 кг</option>' +
        ...
        if($(kind).attr('has_height') == '1')
            $('#filter #height').removeAttr('disabled').html('<option value="0_20">до 20 см</option><option value="21_50">20 - 50 см</option>' +
